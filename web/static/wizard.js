...
async function nextStep(keepNote) {
  ...
  if (res.ok) {
    const resolved = resolveTarget(step, res);
    if (resolved.error) {
      // Retry the same step: the user wants this target mapped, not skipped.
      note(`${resolved.error} Try a button, or Skip.`);
      return nextStep(true);
    }
    const targetName = resolved.target;
    const invert = resolved.invert;
